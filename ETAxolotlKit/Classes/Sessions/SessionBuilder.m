...
#import <ETCurve25519Kit/Curve25519.h>
#import <ETCurve25519Kit/Ed25519.h>
...
- (void)processPrekeyBundle:(PreKeyBundle*)preKeyBundle{
    ...
    ECKeyPair *ourIdentityKeyPair = [self.identityStore identityKeyPair];
...
//        @throw [NSException exceptionWithName:InvalidKeyException reason:@"KeyIsNotValidlySigned" userInfo:nil];
    ...
    if (ourIdentityKeyPair.privateKey.length != ECCKeyLength) {
        @throw [NSException exceptionWithName:UntrustedIdentityKeyException reason:@"Identity key is not valid" userInfo:@{}];
        return;
    }
    ...
    AliceAxolotlParameters *params = [[AliceAxolotlParameters alloc] initWithIdentityKey:ourIdentityKeyPair
    ...
    NSLog(@"setUnacknowledgedPreKeyMessage for: %@ with preKeyId: %d", self.recipientId, theirOneTimePreKeyId);
    ...
    BOOL previousIdentityExisted = [self.identityStore saveRemoteIdentity:theirIdentityKey recipientId:self.recipientId];
...
- (int)processPrekeyV3:(PreKeyWhisperMessage*)message withSession:(SessionRecord*)sessionRecord{
    ...
    BobAxolotlParameters *params = [[BobAxolotlParameters alloc] initWithMyIdentityKeyPair:ourIdentityKeyPair
